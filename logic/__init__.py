...
from utils import (
    normalize_repo_enabled_engines,
    get_threads_config,
    SCHEDULE_TAG,
    get_limit_engines,
    ProjectSchedule,
...
class Scheduler:
    ...
    async def __get_schedule_entry_from_tag(
        self, project_data, schedule_tag_value, bad_cb
    ):
        ...
        repo_details = await ProjectRepoConfig.from_project_json(
            self.__client, project_data
        )

        if await repo_details.is_scm_imported and await repo_details.scm_creds_expired:
            if bad_cb is not None:
                bad_cb(
                    project_data["id"], "Imported repository credentials have expired."
                )
            return None
        ...
        if len(elements) > 0:
            ...
            if ss.is_valid():
                ...
                if engines_from_tag is None:
                    engines = normalize_repo_enabled_engines(
                        await repo_details.get_enabled_scanners(branch)
                    )

                if engines is None or len(engines) == 0:
                    engines = utils.normalize_selected_engines_from_tag(
                        engines_from_tag, await repo_details.is_scm_imported
                    )
                ...
                if repo_url is not None and branch is not None:
                    return {
                        project_data["id"]: [
                            utils.ProjectSchedule(
                                project_data["id"],
                                ss,
                                branch,
                                engines,
                                await repo_details.repo_url,
                            )
                        ]
                    }
            else:
                Scheduler.__log.error(
                    f"Project {project_data['id']}:{project_data['name']} has invalid schedule tag {schedule_tag_value}, skipping."
                )
                if bad_cb is not None:
    ...
    async def __get_selected_engine_array(
        self, repo_cfg: ProjectRepoConfig
    ) -> List[str]:
        ...
        if await repo_cfg.is_scm_imported:
            selected_engines = utils.normalize_selected_engines_from_tag(
                ",".join(
                    await repo_cfg.get_enabled_scanners(await repo_cfg.primary_branch)
                ),
            ...
            if last_scheduled_scan is not None:
                loaded_engine_configs = last_scheduled_scan.get("metadata", {}).get(
                    "configs", []
                )
                selected_engines = utils.normalize_selected_engines_from_tag(
                    ",".join([x.get("type") for x in loaded_engine_configs]),
                    await repo_cfg.is_scm_imported,
                )

            if selected_engines is None:
                selected_engines = utils.normalize_selected_engines_from_tag(
                    None, await repo_cfg.is_scm_imported
                )
    ...
    async def __get_schedule_entry_no_tag(self, bad_cb, group_index, project_json):
        ...
        repo_cfg = await ProjectRepoConfig.from_project_json(
            self.__client, project_json
        )
        if (await repo_cfg.repo_url is not None) and (
            await repo_cfg.primary_branch is not None
        ):
            ...
            if self.__default_schedule is not None and bad_cb is not None:
                bad_cb(
                    project_json["id"],
                    f"Project [{project_json['name']}] has a misconfigured repo url or primary branch.",
                )
    ...
    async def refresh_schedule(self):
        ...
        new_scheduled_projects = set(new_schedule.keys()) - set(
            self.__the_schedule.keys()
        )
        ...
        self.__log.debug(
            f"Detected {len(new_scheduled_projects)} new project schedules"
        )
        ...
        return (
            len(new_scheduled_projects),
            len(removed_projects),
            len(changed_schedule.keys()),
        )
        ...
        async for project in page_generator(
            ...
            **tag_args,
            ...
            if "schedule" in project["tags"].keys():
                entry = await self.__get_schedule_entry_from_tag(
                    project, project["tags"]["schedule"], bad_cb
                )
                ...
                entry = await self.__get_schedule_entry_no_tag(
                    bad_cb, group_index, project
                )
                if entry is not None:
                    schedule.update(entry)

        Scheduler.__log.debug("End: Load project schedule")
        Scheduler.__log.info(
            f"Schedule load time: {timedelta(microseconds=(perf_counter_ns() - load_start)/1000)}"
        )
    ...
    async def __initialize(client, default_schedule, group_schedules, policies):
        ...
        ret_sched.__scheduler = AsyncIOScheduler(
        ...
        for trigger in policies:
            if policies[trigger] not in ret_sched.__trigger_cache.keys():
                ret_sched.__trigger_cache[policies[trigger]] = CronTrigger.from_crontab(
                    policies[trigger]
                )
        ...
        if (
            ...
            and default_schedule in ret_sched.__policies.keys()
        ):
            ...
            Scheduler.__log.error(
                f"Default schedule [{default_schedule}] is not a valid policy."
            )
    ...
    async def audit(client, default_schedule, group_schedules, policies, bad_callback):
        schedule = await Scheduler.__initialize(
            client, default_schedule, group_schedules, policies
        )
    ...
    def __add_job(self, sched: ProjectSchedule) -> None:
        ...
        self.__job_cache[sched.project_id].append(
            self.__scheduler.add_job(
                _exec_wrapper,
                self.__trigger_cache[sched.schedule],
                name=str(sched),
                kwargs={
                    "executor": ScanExecutor(self.__client),
                    "sched": sched,
    ...
    async def start(client, default_schedule, group_schedules, policies):
        ...
        ret_sched = await Scheduler.__initialize(
            client, default_schedule, group_schedules, policies
        )
