...
if sys.argv[0].lower().startswith("audit") or (
    ...
    and sys.argv[1].lower().startswith("audit")
):
    is_audit = True
...
while True:
    try:
        ...
        client = CxOneClient.create_with_oauth(
            oauth_id,
            oauth_secret,
            __agent__,
            auth_endpoint,
        ...
        async def scheduler():
            the_scheduler = await Scheduler.start(
            ...
            while True:
                __log.info(
                    f"Projects with scheduled scans: {the_scheduler.scheduled_scans}"
                )
                ...
                try:
                    ...
                    short_delay = False
                    __log.info(
                        f"Schedule changes: New: {new} Removed: {removed} Changed: {changed}"
                    )
