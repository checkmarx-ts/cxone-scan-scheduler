...
class ScanExecutor:
    ...
    async def __call__(self, sched: ProjectSchedule, threads: asyncio.Semaphore):
        async with threads:
            try:
                project_repo = await ProjectRepoConfig.from_project_id(
                    self.__client, sched.project_id
                )
                safe_name = ScanExecutor.__create_name(
                    project_repo.name, sched.project_id, sched.repo_url, sched.branch
                )

                tag = (
                    {SCHEDULE_TAG: sched.schedule}
                    if sched.schedule is not None
                    else {SCHEDULE_TAG: None}
                )
                ...
                if await self.__should_scan(project_repo, sched.branch):
                    scan_response = await ScanInvoker.scan_by_project_config(
                        self.__client,
                        sched.project_id,
                        sched.branch,
                    ...
                    if scan_response.ok:
                        ...
                        if get_fetch_throttle():
                            ...
                            while not overtime:
                                overtime = (
                                    datetime.now() - timeout_check
                                ).seconds > get_fetch_timeout_config()

                                if scan_insp is None:
                                    if await project_repo.is_scm_imported:
                                        scan_insp = (
                                            await self.__find_scan_for_repo_scan(
                                                sched.project_id, sched.branch
                                            )
                                        )
                                    else:
                                        scan_insp = ScanInspector(scan_response.json())
                                else:
                                    scan_insp = await ScanLoader.load(
                                        self.__client, scan_insp.scan_id
                                    )

                                if scan_insp is not None:
                                    ScanExecutor.log().debug(
                                        f"Throttler monitoring scan {scan_insp.scan_id} for {safe_name}"
                                    )

                                    if not scan_insp.executing:
                                        ScanExecutor.log().debug(
                                            "Scan %s is complete, exiting throttle loop after %d seconds.",
                                            scan_insp.scan_id,
                                            (datetime.now() - timeout_check).seconds,
                                        )
                                        break

                                    if await self.__check_source_fetch_complete(
                                        scan_insp.scan_id
                                    ):
                                        ScanExecutor.log().debug(
                                            "Scan %s indicates source fetch is complete, exiting throttle loop after %d seconds.",
                                            scan_insp.scan_id,
                                            (datetime.now() - timeout_check).seconds,
                                        )
                                        break
                                    ...
                                    ScanExecutor.log().debug(
                                        f"Running scan not found for {safe_name}"
                                    )

                                await asyncio.sleep(ScanExecutor.__CHECK_DELAY_S)

                            if overtime:
                                ScanExecutor.log().warning(
                                    f"Throttle loop is exiting after source fetch timeout for {safe_name}"
                                )
                    else:
                        ScanExecutor.log().error(
                            f"Failed to start scan for project {safe_name}: {scan_response.status_code}:{scan_response.json()}"
                        )

                else:
                    ScanExecutor.log().warning(
                        f"Scheduled scan for {safe_name} skipped."
                    )
    ...
    async def __get_latest_running_scan(
        self, projectid: str, branch: str
    ) -> List[Response]:
    ...
    async def __should_scan(self, project_repo: ProjectRepoConfig, branch: str) -> bool:
        ...
        if not await project_repo.is_scm_imported:
            running_scans = json_on_ok(
                await retrieve_list_of_scans(
                    self.__client,
                    tags_keys=SCHEDULE_TAG,
                    branch=branch,
            ...
            potential_running_scan, potential_queued_scan = (
                await self.__get_latest_running_scan(project_repo.project_id, branch)
            )

            if (
                project_repo.project_id in json_on_ok(potential_running_scan).keys()
                or project_repo.project_id in json_on_ok(potential_queued_scan).keys()
            ):
                running_scan = True

        if not running_scan and get_recent_scan_hours_config() > 0:
            previous_time = datetime.now(timezone.utc) - timedelta(
                hours=get_recent_scan_hours_config()
            )

            previous_scans = json_on_ok(
                await retrieve_list_of_scans(
                    self.__client,
                    branch=branch,
                    project_id=project_repo.project_id,
                    statuses=["Completed"],
                    limit=1,
                    from_date=previous_time.isoformat(),
                )
            )
    ...
    async def __find_scan_for_repo_scan(
        self, projectid: str, branch: str
    ) -> Union[ScanInspector, None]:
